...
8. Every test must contain at least one explicit assertion (`#expect`/`#require` or the language equivalent) on an outcome — state, return value, error, or interaction. Do not throw from a test to signal an expectation failure; use the test framework's assertion APIs so the failure is reported clearly and consistently. If a call offers nothing meaningful to assert beyond "it didn't throw," and there's no idiomatic non-branching way to assert that directly, don't manufacture an assertion around it (e.g. wrapping in `#expect(throws: Never.self) { ... }`, or adding a one-off helper just to produce a boolean) — the test isn't proving anything real, so drop it instead of padding it out.
9. Never branch control flow inside a test body — no `if`, `guard`, `switch` with early return/continue, ternary/conditional expressions, or conditional skips used to decide whether or what an assertion checks. This includes a ternary used to pick an expected value, a branch used to build input, or optional chaining (`?.`) used to shrug off a missing value instead of unwrapping it — each hides a runtime condition inside what should be a fixed, unconditional check. If asserting on a case of an enum, `Result`, or optional requires unwrapping, use the framework's non-branching assertion form (e.g. `#expect(throws:)` around the throwing call, or `try #require(...)` to unwrap unconditionally) instead of pattern-matching, a ternary, or optional chaining with a fallback. Optional chaining is fine only when the chain is asserted against a definite result (e.g. `#expect(value?.field == expected)` where a nil would correctly fail the comparison), never when it silently no-ops on nil. A test's assertions must always execute the same check, not one that depends on a runtime condition.
